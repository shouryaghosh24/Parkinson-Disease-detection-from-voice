...
def extract_features_from_filelike(filelike, n_mfcc=MFCC_N):
    # filelike can be UploadedFile object
...
# ---------------- Scaler utilities ----------------
def build_and_save_scaler(train_folder, scaler_path=SCALER_PATH, n_mfcc=MFCC_N):
    """
    Walks train_folder expecting subfolders 'healthy' and 'parkinson' (or uses all wavs),
    extracts mfcc means, fits StandardScaler, saves to scaler_path.
    """
    X = []
    if not os.path.exists(train_folder):
        return None, f"TRAIN_DATA_DIR '{train_folder}' not found."
    ...
    for root, _, files in os.walk(train_folder):
        for f in files:
            if f.lower().endswith(".wav"):
                ...
                try:
                    mf = extract_mfcc_mean_from_path(p, n_mfcc)
                    X.append(mf)
                except Exception:
                    # skip files that fail
                    continue
    if len(X) == 0:
        return None, "No wav files found while building scaler."
    X = np.vstack(X)  # shape (n_samples, n_mfcc)
    scaler = StandardScaler()
    scaler.fit(X)
    joblib.dump(scaler, scaler_path)
    return scaler, f"Scaler built from {len(X)} files and saved to {scaler_path}."

def load_or_build_scaler(scaler_path=SCALER_PATH, train_folder=TRAIN_DATA_DIR):
    if os.path.exists(scaler_path):
        try:
            scaler = joblib.load(scaler_path)
            return scaler, f"Loaded scaler from {scaler_path}"
        except Exception as e:
            # fallthrough to build
            pass
    # try building
    scaler, msg = build_and_save_scaler(train_folder, scaler_path)
    return scaler, msg

# ---------------- Get scaler ready ----------------
scaler, scaler_msg = load_or_build_scaler()
if scaler is None:
    st.warning("Scaler not available and could not be built from TRAIN_DATA_DIR. Using fallback normalization (z-score on single sample).")
else:
    st.success(scaler_msg)

# ---------------- PDF report (keeps previous implementation) ----------------
...
if uploaded_file is not None:
    ...
    try:
        # extract features (MFCC mean and display params)
